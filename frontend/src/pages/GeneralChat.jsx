    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100 flex flex-col transition-colors duration-300">
      ...
      <nav className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-20 shadow-sm transition-colors duration-300">
        <div className="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button onClick={() => navigate('/')} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white cursor-pointer">
              <ArrowLeft className="w-5 h-5" />
            </button>
            <div className="flex items-center gap-2 text-xl font-bold tracking-tight text-slate-850 dark:text-white">
              <Scale className="text-nyaya-500 w-6 h-6" /> NyayaVanni <span className="text-slate-400 dark:text-slate-500 font-medium hidden sm:inline">| Assistant</span>
            </div>
          </div>
          <ThemeToggle />
        </div>
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto p-4 sm:p-6 flex flex-col">
        ...
        <div className="flex-1 bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col h-[calc(100vh-8rem)] transition-colors duration-300">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 bg-slate-50/50 dark:bg-slate-950/20">
            {chatHistory.map((msg, idx) => (
              <div key={idx} className={`flex gap-3 sm:gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                ...
                {/* Avatar Icon Wrapper */}
                <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center shrink-0 ${msg.role === 'user' ? 'bg-nyaya-500 text-white shadow-md' : 'bg-slate-100 dark:bg-slate-850 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}>
                  {msg.role === 'user' ? <User className="w-4 h-4 sm:w-5 sm:h-5" /> : <Bot className="w-5 h-5 sm:w-6 sm:h-6" />}
                </div>
                
                {/* Message Bubble Grid */}
                <div className={`p-4 rounded-2xl max-w-[85%] sm:max-w-[75%] text-sm sm:text-base leading-relaxed whitespace-pre-wrap ${msg.role === 'user' ? 'bg-nyaya-900 text-white rounded-tr-sm shadow-md border border-nyaya-800' : 'bg-slate-100 dark:bg-slate-850/80 border border-slate-200 dark:border-slate-800 rounded-tl-sm text-slate-700 dark:text-slate-200 shadow-sm'}`}>
                  {msg.role === 'assistant' ? (
                    /* Typography modifications for Markdown readability */
                    <div className="prose prose-sm max-w-none prose-li:my-0.5 prose-ul:my-1 prose-p:my-1 text-slate-750 dark:text-slate-200 prose-headings:text-slate-850 dark:prose-headings:text-white prose-strong:text-nyaya-600 dark:prose-strong:text-nyaya-400 prose-code:text-amber-600 dark:prose-code:text-amber-250">
            ...
            {chatLoading && (
              <div className="flex gap-3 sm:gap-4">
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-slate-100 dark:bg-slate-850 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 flex items-center justify-center shrink-0">
          ...
          <form onSubmit={handleChat} className="p-3 sm:p-4 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex gap-2 sm:gap-3 transition-colors duration-300">
            <input 
              type="text" 
              value={chatInput}
      ...
      <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 shrink-0 pb-6">
        <Footer />
      </div>
    </div>
  );
